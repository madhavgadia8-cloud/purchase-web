      ...
      <div className="wrap">
        {dbError ? (
          <div className="card">
            <h2>Setup needed</h2>
            <p className="muted">
              Could not reach the database: {dbError}. Check your Supabase env vars and that you ran
              the schema SQL.
            </p>
          </div>
        ) : null}
        ...
        <div className="card">
          <h2>Create a purchase requirement (RFQ)</h2>
          <NewRfqForm />
        </div>
              ...
              return (
                <div className="listitem" key={r.id}>
                  <div>
                    ...
                    <div className="muted">
                      {items} item(s)
                      {r.required_by ? ` · by ${r.required_by}` : ""} · {quotes} quote(s) received
                    </div>
                  </div>
                  <Link className="btn ghost sm" href={`/rfq/${r.id}`}>
                    Open
                  </Link>
                </div>
              );
